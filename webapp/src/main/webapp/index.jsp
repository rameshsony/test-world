<Html>    
<Head>  
<title>  
Example of center and BR tag  
</title>  
</Head>  
<Body> 
<body style="background-color:powderblue;">
<!-- In this example we use the center tag which specify the content at centre of the webpage.-->    
<center>  
<h1>Welcome to RS & Ashwik Technologies...!</h1> <br> <!-- The BR tag is used to break a line. -->  
 <h2>For DevOps learning</h2> <br>   
Here you can master in all DevOps Tools
 <h3> we sucessfully Deployed our web application on tomcat server </h3>
  <h3> we sucessfully Deployed our web application on tomcat server </h3>
 <!-- Here BR tag is not used, so the next statement is in continuous with one space after this statement. --> 
</center>
 <br>  
</Body>  
</Html>  

















